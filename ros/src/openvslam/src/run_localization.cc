...
void mono_localization(const std::shared_ptr<PLPSLAM::config> &cfg, const std::string &vocab_file_path,
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        cfg = std::make_shared<PLPSLAM::config>(setting_file_path->value());
    }
    catch (const std::exception &e)
    {
        std::cerr << e.what() << std::endl;
