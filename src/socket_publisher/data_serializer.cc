...
namespace socket_publisher
{
    ...
    data_serializer::data_serializer(const std::shared_ptr<PLPSLAM::publish::frame_publisher> &frame_publisher,
    ...
    {
        std::vector<PLPSLAM::data::keyframe *> keyframes;
        map_publisher_->get_keyframes(keyframes);

        std::vector<PLPSLAM::data::landmark *> all_landmarks;
    ...
    std::string data_serializer::serialize_as_protobuf(const std::vector<PLPSLAM::data::keyframe *> &keyfrms,
                                                       const std::vector<PLPSLAM::data::landmark *> &all_landmarks,
                                                       const std::set<PLPSLAM::data::landmark *> &local_landmarks,
                                                       const PLPSLAM::Mat44_t &current_camera_pose)
    {
        map_segment::map map;
        auto message = map.add_messages();
        message->set_tag("0");
