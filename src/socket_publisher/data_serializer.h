...
namespace socket_publisher
{
    ...
    {
    public:
        data_serializer(const std::shared_ptr<PLPSLAM::publish::frame_publisher> &frame_publisher,
    ...
    private:
        ...
        inline double get_vec_hash(const PLPSLAM::Vec3_t &point)
        {
            return point[0] + point[1] + point[2];
        }

        inline double get_mat_hash(const PLPSLAM::Mat44_t &pose)
        {
            return pose(0, 3) + pose(1, 3) + pose(2, 3);
        }

        std::string serialize_as_protobuf(const std::vector<PLPSLAM::data::keyframe *> &keyfrms,
